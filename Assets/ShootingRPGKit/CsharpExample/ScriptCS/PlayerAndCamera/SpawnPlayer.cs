...
public class SpawnPlayer : MonoBehaviour {
	...
	public bool autoLoadTempData = false; //Auto load character data from Temporary Save Data.
	...
	void  Awake (){
		...
		if(currentPlayer){
			...
			GameObject oldCam = currentPlayer.GetComponent<GunTrigger>().mainCam;
			if(!oldCam){
				return;
			}
		...
		if(autoLoadTempData && characterDatabase){
			...
			Instantiate(playerPrefab[spawnId], transform.position , transform.rotation);
		}
	}
