...
function OnGUI(){
	...
	//Shop Main Menu
	if(menu && shopMain){
		GUI.Box (Rect (Screen.width / 2 - 200,180,400,140), "Shop");
		if (GUI.Button (Rect (Screen.width / 2 - 160,240,150,60), "Buy")) {
			shopItem = true;
			shopMain = false;
		}
		if (GUI.Button (Rect (Screen.width / 2 + 10,240,150,60), "Sell")) {
			itemInven = true;
			shopMain = false;
		}
		if (GUI.Button (Rect (Screen.width / 2 + 165,190,30,30), "X")) {
			OnOffMenu();
		}
	}
...
function BuyItemWindow(windowID : int){
		//Close Window Button
		if (GUI.Button (Rect (420,8,40,40), "X")) {
			OnOffMenu();
		}
...
@script AddComponentMenu ("Shooting RPG Kit/Shop")
