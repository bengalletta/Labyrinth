...
#pragma implicit
#pragma downcast
var freezeGravity : boolean = false;

// Does this script currently respond to input?
var canControl : boolean = true;
var limitVelocityY : float = 7.0;

var useFixedUpdate : boolean = true;

var airJump = 0;
private var airJumpTime = 0;
...
private function UpdateFunction () {
	...
	// We were not grounded but just landed on something
		...
		/*if(playeranim){
			//GetComponent(PlayerInputController).PlayLandingAnim();
			SendMessage("PlayLandingAnim");
		}*/
	}
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded){
		...
		//--------Buju
		/*if (Input.GetButtonDown("Jump") && airJump > 0 && airJumpTime < airJump) {
				// Negate the gravity we just applied, except we push in jumpDir rather than jump upwards.
				//velocity += jumping.jumpDir * movement.gravity * Time.deltaTime;
				velocity.y = 0;
				velocity += jumping.jumpDir * CalculateJumpVerticalSpeed (jumping.baseHeight *2);
				airJumpTime++;
