...
var autoAssignWeapon : boolean = false; // Use for Auto Assign Weapon Equip to Gun Trigger.
var itemSlot : int[] = new int[18];
var itemQuantity : int[] = new int[18];
var equipment : int[] = new int[15];
var equipAmmo : int[] = new int[15];

var primaryEquip : int = 0;
var secondaryEquip : int = 0;
var meleeEquip : int = 0;
...
function UseItem(slot : int){
	var id : int = itemSlot[slot];
...
function EquipItem(id : int , slot : int){
	...
	if(dataItem.equipment[id].equipmentType == EqType.PrimaryWeapon){
		...
		if(dataItem.equipment[id].model && gun.weaponPosition){
		    ...
		    var wea : GameObject = Instantiate(dataItem.equipment[id].model,gun.weaponPosition.position,gun.weaponPosition.rotation);
		...
		if(dataItem.equipment[id].model && gun.weaponPosition){
		    ...
		    wea = Instantiate(dataItem.equipment[id].model,gun.weaponPosition.position,gun.weaponPosition.rotation);
	   		wea.transform.parent = gun.weaponPosition;
		...
		//Change Weapon Mesh
		if(dataItem.equipment[id].model && gun.weaponPosition){
		    wea = Instantiate(dataItem.equipment[id].model,gun.weaponPosition.transform.position,gun.weaponPosition.transform.rotation);
	   		wea.transform.parent = gun.weaponPosition;
...
function UnEquip(id : int){
	if(dataItem.equipment[id].equipmentType == EqType.PrimaryWeapon){
			var full : boolean = AddEquipment(primaryEquip , gun.primaryWeapon.ammo);
			//-----------------------------------
	}else if(dataItem.equipment[id].equipmentType == EqType.SecondaryWeapon){
...
function AddEquipment(id : int , ammo1 : int) : boolean{
	var full : boolean = false;
	var geta : boolean = false;
...
function SetWeaponValue(id : int){
	var equipType : int = dataItem.equipment[id].equipmentType;
...
function AssignWeaponAnimation(id : int){
	...
	if(equipType == 2 && dataItem.equipment[id].attackAnimation.Length > 0){
  		var allPrefab : int = dataItem.equipment[id].attackAnimation.length;
 	...
 	var playerAnim : PlayerLegacyAnimation = GetComponent(PlayerLegacyAnimation);
	if(!playerAnim){
		//If use Mecanim
...
function AssignMecanimAnimation(id : int , type : int){
